...
public class BoxService implements BoxFactory, Placeable<Box> {
	...
	/*
	 * private boolean checkIfCanBePlaced(Vector2 vec){
	 * if(tiledMapTileLayer.getCell((int)vec.x,(int)vec.y).getTile().getId() ==
	 * 83){ for(int i = 0 ; i < tiledMapTileLayer.getWidth()/16 ; i++){ for(int
	 * j = 0 ; j < tiledMapTileLayer.getHeight()/16 ; j++){
	 * if(tiledMapTileLayer.getCell((int)vec.x,(int)vec.y).getTile().getId() ==
	 * 83){ return true; }else{ vec.x = + 16*i; vec.y = + 16*j; } } } } }
	 */
